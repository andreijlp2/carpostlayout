...
const Footer = () => {
  return (
    <footer className="border-t border-border bg-[hsl(240,30%,10%)] py-12 lg:py-16">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {["Funcionalidades", "Planos", "Integrações", "API"].map((l) => (
                <li key={l}>
                  <a href="#" className="text-sm text-white/50 hover:text-accent transition-colors">{l}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="border-t border-white/10 pt-6 sm:pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          ...
          <div className="flex gap-4">
            {["Instagram", "LinkedIn", "YouTube"].map((s) => (
              <a key={s} href="#" className="text-sm text-white/40 hover:text-accent transition-colors">
                {s}
              </a>
            ))}
          </div>
