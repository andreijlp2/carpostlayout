      ...
      <div className="container mx-auto flex h-16 sm:h-20 lg:h-24 items-center justify-between px-4 lg:px-8">
        ...
        <div className="hidden md:flex items-center gap-8">
          {["funcionalidades", "como-funciona", "planos", "depoimentos"].map((id) => {
            ...
            return (
              <a key={id} href={`#${id}`} className={`text-sm transition-colors ${scrolled ? "text-muted-foreground hover:text-foreground" : "text-foreground/70 hover:text-foreground"}`}>
                {labels[id]}
              </a>
            );
          })}
        ...
        <div className="hidden md:flex items-center gap-3">
          <Button variant="ghost" size="sm" className={scrolled ? "text-muted-foreground hover:text-foreground" : "text-foreground/70 hover:text-foreground"}>
            Entrar
          </Button>
          <Button size="sm" className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-md font-semibold">
            Começar agora
          </Button>
        </div>
