  ...
  return (
    <section className="py-16 lg:py-20 bg-secondary/30">
      <div className="container mx-auto px-4 lg:px-8">
        <motion.p
          className="text-center text-sm font-semibold uppercase tracking-widest text-muted-foreground mb-12"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true }}
          variants={fadeInUp}
      ...
      <div className="relative overflow-hidden group">
        <div className="absolute left-0 top-0 bottom-0 w-28 bg-gradient-to-r from-secondary/30 to-transparent z-10 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-28 bg-gradient-to-l from-secondary/30 to-transparent z-10 pointer-events-none" />

        <div className="flex animate-marquee group-hover:[animation-play-state:paused] w-max items-center">
          {doubledLogos.map((logo, i) => (
            <div
              key={i}
              className="flex-shrink-0 mx-5 sm:mx-7 flex items-center justify-center w-36 h-12 px-2 grayscale opacity-45 hover:grayscale-0 hover:opacity-100 transition-all duration-400 cursor-default"
            >
              <img
                src={logo.src}
                alt={logo.name}
